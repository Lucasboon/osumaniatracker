...
function loadAndDisplayStats() {
    ...
    if (savedData) {
        ...
        document.getElementById('recent-plays-list').innerHTML = '<div class="text-center py-8 text-zinc-500 text-sm">Click refresh to load your played maps and scores.</div>';
    }
}
...
async function refreshMyStats() {
    ...
    if (apiKey) {
        ...
        // Also fetch recent plays (what songs played + scores)
        if (newData) {
            newData.recent_plays = await fetchRecentPlays(myProfile.username, myProfile.mode);
        }
    }
    ...
    displayRecentPlays(newData.recent_plays || []);
    
    btns.forEach(b => b.disabled = false);
    
    const toast = document.createElement('div');
    toast.className = 'fixed bottom-6 right-6 bg-emerald-600 text-white px-6 py-3 rounded-2xl shadow-xl';
    toast.textContent = apiKey ? 'Real data + recent plays refreshed!' : 'Demo data refreshed';
...
async function fetchRecentPlays(username, mode) {
    if (!apiKey) return [];
    
    const modeMap = { osu: 0, taiko: 1, catch: 2, mania: 3 };
    const m = modeMap[mode] || 0;
    
    const url = `https://osu.ppy.sh/api/get_user_recent?k=${apiKey}&u=${encodeURIComponent(username)}&m=${m}&limit=15`;
    const proxy = `https://corsproxy.io/?${encodeURIComponent(url)}`;
    ...
    try {
        ...
        return data.map(play => ({
            beatmap: play.beatmapset?.title 
                ? `${play.beatmapset.artist} - ${play.beatmapset.title} [${play.beatmap?.version || ''}]`
                : 'Unknown Map',
            score: parseInt(play.score) || 0,
            pp: parseFloat(play.pp) || 0,
            accuracy: (parseFloat(play.accuracy) * 100).toFixed(2),
            mods: play.enabled_mods || 'None',
            date: play.date,
            rank: play.rank || '?'
        }));
    } catch (e) {
...
// Display recent plays (songs played + scores on each map)
function displayRecentPlays(plays) {
    const container = document.getElementById('recent-plays-list');
    const countEl = document.getElementById('recent-count');
    
    if (!plays || plays.length === 0) {
        container.innerHTML = '<div class="text-center py-8 text-zinc-500 text-sm">No recent plays loaded yet. Refresh to see songs and map scores.</div>';
        countEl.textContent = '0 plays';
        return;
    }
    
    countEl.textContent = `${plays.length} recent plays`;
    
    let html = '';
    
    plays.forEach(play => {
        const date = play.date ? new Date(play.date).toLocaleDateString('en-US', {month:'short', day:'numeric'}) : 'Recently';
        
        html += `
            <div class="bg-zinc-800 border border-zinc-700 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div class="flex-1 min-w-0">
                    <div class="font-medium text-white truncate">${play.beatmap}</div>
                    <div class="text-xs text-zinc-400 mt-1 flex items-center gap-x-2">
                        <span>${date}</span>
                        <span class="px-2 py-px bg-zinc-700 rounded text-[10px]">${play.mods}</span>
                    </div>
                </div>
                
                <div class="flex items-center gap-x-6 text-sm">
                    <div class="text-right">
                        <div class="font-mono text-pink-400">${play.pp} <span class="text-xs text-zinc-400">pp</span></div>
                        <div class="text-xs text-zinc-400">${play.accuracy}% acc</div>
                    </div>
                    
                    <div class="text-right">
                        <div class="font-mono">${play.score.toLocaleString()}</div>
                        <div class="text-xs text-emerald-400">${play.rank}</div>
                    </div>
                </div>
            </div>
        `;
